...
def init():
    ...
    command("F100")     # feed at 100 mm/min
...
def rotate_point(point, centre, theta):
    """Rotate the point clockwise about the centre by theta radians."""
    pass
...
def circle(centre, radius, depth, start_depth=0.0, offset=None, tab=False):
    ...
    while curr_depth < depth:
        ...
        if not (last_pass and tab):
            ...
            # Current implementation is hacky and repetetive, but works ;)
...
def fillcircle(centre, radius, depth, start_depth=0.0):
    """Completely mill out a circle.
    Start with a minimum circle at the centre, and work outwards.
    offset is automatically considered to be 'inside'.
    """
    radius -= tool_width / 2
    radii = frange(tool_width / 2, radius, step_over)
    
    for temp_radius in radii:
        # inlining the circle part, since we don't want to move
        # back up to safe height each time
        #circle(centre, temp_radius, depth, offset='inside')

        curr_depth = start_depth
        # start at the left hand side of the circle
        start = (centre[0] - temp_radius, centre[1])
        move(start)
        ...
        while curr_depth < depth:
            ...
            # (G-Code Sender) won't show it. So - one pass to the opp.
            # side, and another back
            command("G2 X%.03f Y%.03f I%.03f J0" % (start[0] + 2 * temp_radius, start[1], temp_radius))
            command("G2 X%.03f Y%.03f I-%.03f J0" % (start[0], start[1], temp_radius))

        # move to just above the surface
        command('G1 Z%.03f' % (start_depth + 1.0))
