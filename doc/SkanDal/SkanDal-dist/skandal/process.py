...
def lines_image(x_array, y_array, queue, blackim):
    ##last = np.concatenate(x_array, y_array).reshape(x_array.shape[0], 2)
    ##queue = np.concatenate(queue, last)
    ##queue =
    im = blackim#.copy()
    for i in range(x_array.shape[0]):
        im.itemset(y_array[i], x_array[i], 1, i%255)
        im.itemset(y_array[i], x_array[i], 2, 255-i%255)
    return im, queue
